  ...
  'daily-sales': {
    ...
    columns: [
      { key: 'date', header: 'Date', render: (row) => formatDate(row.date as string) },
      {
        key: 'totalSales',
        header: 'Total Sales',
        render: (row) => <Amount value={Number(row.totalSales)} />,
      },
      { key: 'billCount', header: 'Bills', render: (row) => String(row.billCount) },
      {
        key: 'returns',
        header: 'Returns',
        render: (row) => <Amount value={Number(row.returns)} />,
      },
      {
        key: 'netSales',
        header: 'Net Sales',
        render: (row) => <Amount value={Number(row.netSales)} />,
      },
      {
        key: 'cashSales',
        header: 'Cash',
        render: (row) => <Amount value={Number(row.cashSales)} />,
        hideOnMobile: true,
      },
      {
        key: 'upiSales',
        header: 'UPI',
        render: (row) => <Amount value={Number(row.upiSales)} />,
        hideOnMobile: true,
      },
    ],
  },
  ...
  'sales-by-category': {
    ...
    columns: [
      ...
      { key: 'billCount', header: 'Bills', render: (row) => String(row.billCount) },
      {
        ...
        render: (row) => <Amount value={Number(row.avgBillValue)} />,
        hideOnMobile: true,
      },
      {
        key: 'percentage',
        header: '% Share',
        render: (row) => `${Number(row.percentage).toFixed(1)}%`,
        hideOnMobile: true,
      },
    ],
  },
  ...
  'sales-by-brand': {
    ...
    columns: [
      ...
      { key: 'billCount', header: 'Bills', render: (row) => String(row.billCount) },
      {
        key: 'avgSellingPrice',
        header: 'Avg Price',
        render: (row) => <Amount value={Number(row.avgSellingPrice)} />,
        hideOnMobile: true,
      },
      {
        key: 'percentage',
  ...
  'inventory-valuation': {
    ...
    columns: [
      ...
      { key: 'currentStock', header: 'Qty', render: (row) => String(row.currentStock) },
      {
        key: 'costPrice',
        header: 'Cost Price',
        render: (row) => <Amount value={Number(row.costPrice)} />,
      },
      {
        key: 'totalValue',
        header: 'Total Value',
        render: (row) => <Amount value={Number(row.totalValue)} />,
      },
      {
        key: 'categoryName',
        header: 'Category',
        render: (row) => String(row.categoryName),
        hideOnMobile: true,
      },
    ],
  },
  ...
  'aging-inventory': {
    ...
    columns: [
      ...
      { key: 'currentStock', header: 'Qty', render: (row) => String(row.currentStock) },
      {
        ...
        render: (row) => String(row.daysSinceLastSale),
      },
      {
        key: 'lastSaleDate',
        header: 'Last Sale',
        render: (row) => (row.lastSaleDate ? formatDate(row.lastSaleDate as string) : 'Never'),
      },
      {
        key: 'totalValue',
        header: 'Value',
  ...
  'dead-stock': {
    ...
    columns: [
      ...
      },
      {
        ...
        render: (row) => String(row.daysSinceReceived),
        hideOnMobile: true,
      },
    ],
  },
  ...
  'supplier-ledger': {
    ...
    columns: [
      ...
      { key: 'description', header: 'Description', render: (row) => String(row.description) },
      {
        key: 'referenceId',
        header: 'Reference',
        render: (row) => String(row.referenceId ?? '--'),
        hideOnMobile: true,
      },
      {
        key: 'debit',
        header: 'Debit',
        render: (row) => (row.debit ? <Amount value={Number(row.debit)} /> : <>--</>),
      },
      {
        key: 'credit',
        header: 'Credit',
        render: (row) => (row.credit ? <Amount value={Number(row.credit)} /> : <>--</>),
      },
      {
        key: 'runningBalance',
        header: 'Balance',
        render: (row) => <Amount value={Number(row.runningBalance)} />,
      },
    ],
  },
  ...
  'outstanding-payables': {
    ...
    columns: [
      { key: 'supplierName', header: 'Supplier', render: (row) => String(row.supplierName) },
      {
        ...
        render: (row) => <Amount value={Number(row.totalPurchases)} />,
      },
      {
        key: 'totalPayments',
        header: 'Total Paid',
        render: (row) => <Amount value={Number(row.totalPayments)} />,
      },
      {
        key: 'outstanding',
        header: 'Outstanding',
        render: (row) => <Amount value={Number(row.outstanding)} />,
      },
      {
        key: 'lastPaymentDate',
        header: 'Last Payment',
        render: (row) =>
          row.lastPaymentDate ? formatDate(row.lastPaymentDate as string) : 'Never',
        hideOnMobile: true,
      },
      {
        key: 'agingDays',
        header: 'Aging (Days)',
        render: (row) => String(row.agingDays ?? '--'),
        hideOnMobile: true,
      },
    ],
  },
  ...
  pnl: {
    ...
    columns: [
      ...
      },
      {
        ...
        render: (row) =>
          row.previousAmount != null ? <Amount value={Number(row.previousAmount)} /> : <>--</>,
        hideOnMobile: true,
      },
      {
        key: 'change',
        header: 'Change',
        render: (row) => (row.change != null ? `${Number(row.change).toFixed(1)}%` : '--'),
        hideOnMobile: true,
      },
    ],
  },
  ...
  'bargain-discount': {
    ...
    columns: [
      ...
      { key: 'date', header: 'Date', render: (row) => formatDate(row.date as string) },
      {
        key: 'customerName',
        header: 'Customer',
        render: (row) => String(row.customerName ?? 'Walk-in'),
      },
      {
        key: 'subtotal',
        header: 'Subtotal',
        render: (row) => <Amount value={Number(row.subtotal)} />,
        hideOnMobile: true,
      },
      {
        key: 'discountAmount',
        header: 'Discount',
        render: (row) => <Amount value={Number(row.discountAmount)} />,
      },
      {
        key: 'discountPct',
        header: 'Discount %',
        render: (row) => `${Number(row.discountPct).toFixed(1)}%`,
      },
      {
        key: 'salesperson',
        header: 'Salesperson',
  ...
  'cash-register': {
    ...
    columns: [
      ...
      { key: 'userName', header: 'User', render: (row) => String(row.userName) },
      {
        key: 'openingBalance',
        header: 'Opening',
        render: (row) => <Amount value={Number(row.openingBalance)} />,
      },
      { key: 'cashIn', header: 'Cash In', render: (row) => <Amount value={Number(row.cashIn)} /> },
      {
        key: 'cashOut',
        header: 'Cash Out',
        render: (row) => <Amount value={Number(row.cashOut)} />,
      },
      {
        key: 'closingBalance',
        header: 'Closing',
        render: (row) => <Amount value={Number(row.closingBalance)} />,
      },
      {
        key: 'discrepancy',
        header: 'Discrepancy',
        render: (row) => <Amount value={Number(row.discrepancy)} />,
        hideOnMobile: true,
      },
    ],
  },
  ...
  'purchase-summary': {
    ...
    columns: [
      ...
      { key: 'invoiceCount', header: 'Invoices', render: (row) => String(row.invoiceCount) },
      {
        key: 'totalAmount',
        header: 'Total Amount',
        render: (row) => <Amount value={Number(row.totalAmount)} />,
      },
      { key: 'totalTax', header: 'Tax', render: (row) => <Amount value={Number(row.totalTax)} /> },
      {
        key: 'totalPaid',
        header: 'Paid',
        render: (row) => <Amount value={Number(row.totalPaid)} />,
        hideOnMobile: true,
      },
      {
        key: 'outstanding',
    ...
    columns: [
      ...
      { key: 'type', header: 'Type', render: (row) => String(row.type) },
      {
        ...
        render: (row) => {
          ...
          return (
            <span className={val > 0 ? 'text-emerald-600' : 'text-destructive'}>
...
function ReportViewerPage() {
  ...
  if (dateRange.from) filters.from = dateRange.from.toISOString()
  if (dateRange.to) filters.to = dateRange.to.toISOString()
  ...
  const rows =
    reportData?.rows ?? (Array.isArray(reportData) ? (reportData as Record<string, unknown>[]) : [])
